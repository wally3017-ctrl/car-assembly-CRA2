...
class ConsoleUI {
public:
    void showCarTypeMenu()        const;  // "0. 종료" 항목 포함
    ...
    int  readInt(int min, int max);  // 범위 검증 + "exit" 문자열 처리 포함

    bool isExitRequested() const;
    void requestExit();              // CarAssembler에서 0번 선택 시 호출
};
```

**첫 화면 종료 옵션 (`2-6a`)**

차량 타입 선택 화면에 `0. 종료`를 추가하여 프로그램을 명시적으로 종료할 수 있도록 한다.  
`CarAssembler::buildCar()`가 `readInt(0, 3)`으로 0을 수용하고, 선택 시 `ui_.requestExit()`를 호출하여 `run()` 루프를 종료한다.
...
```cpp
// CarAssembler.cpp — Step::CAR_TYPE
